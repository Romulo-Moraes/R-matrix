...
fn main(){
    ...
    program_configurations::set_signals_handling();
    ...
    thread::spawn(move || matrix_renderer(matrix_clone, remaining_chars_of_columns_clone, empty_positions_flags_clone, available_matrix_chars, handled_program_arguments_clone));

    // Starting entry buffer controller
    // This code segment will create new strings to fall in matrix
    controllers::entry_buffer_controller(matrix.clone() ,empty_positions_flags.clone(), remaining_chars_of_columns.clone(), term_size.clone(), handled_program_arguments.clone());
}